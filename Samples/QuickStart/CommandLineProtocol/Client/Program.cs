...
namespace CommandLineClient {
    class Program {
        ...
        static public void Main() {
            ...
            var client = new SocketClient<ISyncMessageInfo<CommandLineMessage>, CommandLineMessage>(new SyncClientCommandLineProtocolHandlerFactory(), 102400, 102400, 30000, 30000);
            client.ReceivedAsyncMessage += (connection, msg) => {
                if (msg.FullCommandLine.Length > 20) {//"init_reply ok"
                    ReceivedAsyncMessages.Add(msg.FullCommandLine);
                    //if (ReceivedAsyncMessages.Count % 3000 == 0) 
                    Console.WriteLine($"client {connection.ConnectionID} ReceivedAsyncMessage:\t{msg}");

                } else {
                    ReceivedAsyncMessages2.Add(msg.FullCommandLine);
                }
            };

            //建立50个socket连接
            for (int i = 0; i < 50; i++) {
                client.TryRegisterEndPoint(i.ToString(), new[] { new IPEndPoint(IPAddress.Parse("127.0.0.1"), 19510) },//     //116.62.105.231
                    (connection, name) => {
                        var source = new TaskCompletionSource<bool>();
                        client.SendSync(new CommandLineMessage("init"),
        ...
        static private void Do(SocketClient<ISyncMessageInfo<CommandLineMessage>, CommandLineMessage> client, int count) {
        ...
        static public void Echo(SocketClient<ISyncMessageInfo<CommandLineMessage>, CommandLineMessage> client, long seqId) {
            var seq = $"{seqId:0000000}";
            var guid = Guid.NewGuid().ToString();
            var task = client.SendSync(new CommandLineMessage("echo", new string[] { seq, guid }),
